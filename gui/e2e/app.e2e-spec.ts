...
describe('Pipes', function () {
  ...
  it('should open correctly', function () {
    ...
    expect(element(by.css('app-node-birthday p')).getText()).toEqual(`The hero's birthday is Apr 15, 1988`);
  });

  it('should show 4 heroes', function () {
    expect(element.all(by.css('app-node-list div')).count()).toEqual(4);
  });

  it('should show a familiar hero in json', function () {
    expect(element(by.cssContainingText('app-node-list p', 'Heroes as JSON')).getText()).toContain('Bombasto');
  });
